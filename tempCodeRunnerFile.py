...
app = Flask(__name__)
CORS(app, resources={r"/*": {"origins": "*"}})  # Güvenlik için origins'i chrome-extension://[uzantı_id] ile sınırlandırabilirsiniz
...
def get_db_connection():
    """Veritabanı bağlantısını güvenli bir şekilde açar."""
    try:
...
def init_db():
    ...
    try:
        ...
        # Mevcut tablolara yeni sütunlar ekleme (gerekirse)
        try:
...
def temizle_metin(metin):
    """Metni temizler ve analiz için hazırlar."""
    try:
        metin = metin.lower()
...
def predict():
    """Metin analizi yapar ve sonucu kaydeder."""
    try:
        data = request.get_json()
...
def gecmis_analizler():
    """Kullanıcının geçmiş analizlerini döndürür."""
    try:
        data = request.get_json()
        user_id = data.get('user_id')
...
def delete_user():
    """Kullanıcıyı ve analizlerini siler."""
    try:
        data = request.get_json()
        user_id = data.get('user_id')
...
def veriler_data():
    """Son analizi döndürür (admin paneli için)."""
    try:
        conn = get_db_connection()
        c = conn.cursor()
...
@app.route('/')
def index():
    """Ana sayfa şablonunu döndürür."""
    return render_template('index.html')

@app.route('/veriler')
def veriler():
    """Veriler sayfasını döndürür (admin paneli)."""
