...
from sklearn.tree import DecisionTreeClassifier  # Decision Tree sınıfı import edildi
...
pipeline = Pipeline([
    ...
    ('dt', DecisionTreeClassifier(random_state=42))  # Decision Tree modelini kullandık
])
...
param_grid = {
    ...
    'dt__max_depth': [10, 20, 30],  # Karar ağacının derinliği
    'dt__min_samples_split': [2, 5, 10],  # Düğüm bölünmeleri için minimum örnek sayısı
    'dt__min_samples_leaf': [1, 2, 4],  # Yapraklar için minimum örnek sayısı
    'dt__criterion': ['gini', 'entropy']  # Kullanılacak kriterler (Gini veya Entropi)
}
...
joblib.dump(grid_search.best_estimator_, 'csv_dosyaları/en_iyi_model_dt.pkl')

# 9. Modeli Yükleyip Test Etme (isteğe bağlı)
loaded_model = joblib.load('csv_dosyaları/en_iyi_model_dt.pkl')
